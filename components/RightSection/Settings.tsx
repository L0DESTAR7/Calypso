...
export default function Settings() {
  ...
  function enableShowSettings() {
    setShowSettings(true);
  }

  function disableShowSettings() {
  ...
  return (
    <div className={`flex flex-row items-center`}>
      {
        <button onClick={disableShowSettings} disabled={!showSettings}>
          <div className={`transition-all duration-700 flex flex-col w-7 h-11 bg-bunker-500 rounded-l-lg items-center
          ${showSettings ?
              'opacity-100'
              :
      ...
      <button onClick={enableShowSettings}>
        <div className={`transition-all duration-1000
        ${!showSettings ?
            'flex flex-row w-12 h-12 self-center place-self-end rounded-l-md place-items-center bg-bunker-500'
            :
            'w-48 h-48 bg-bunker-500 rounded-l-3xl'
          }`}>
          {!showSettings ?
            <svg width="25" height="25" fill="none" viewBox="0 0 25 25" className={`mx-auto animate-spinslow transition-all duration-1000
          ${!showSettings ?
                'opacity-100'
                :
